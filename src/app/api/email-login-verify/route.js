...
export async function GET(req) {
  ...
  try {
    ...
    const { data: su } = await supa.from("site_users").select("permissions, is_admin").eq("email", data.email).maybeSingle();
  ...
  const session = signSession({ email: data.email, name: data.email.split("@")[0], ...(perms.length ? { perms } : {}), exp: Date.now() + 30 * 24 * 3600 * 1000 });
