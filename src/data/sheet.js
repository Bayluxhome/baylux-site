...
// Чистим описание от телеграм-мусора: markdown-ссылки [текст](url), форматирование (**, __, *),
// квадратные скобки и пустые (), а также строки с контактами/призывами (ссылки, @ники, телефоны, «в личку»).
// Применяется и на импорте, и на чтении — поэтому старые объявления тоже показываются чисто.
// Скобки с содержимым, например «(вид на море)», сохраняются.
export function cleanDesc(raw) {
  let s = String(raw == null ? "" : raw).replace(/ /g, " ");
  s = s.replace(/\[[^\]]*\]\([^)]*\)/g, " "); // markdown-ссылка [текст](url) — убрать целиком
  s = s.replace(/[*_~`]+/g, "");              // markdown-форматирование
  s = s.replace(/\[\s*\]|\(\s*\)/g, " ");     // пустые скобки [] ()
  s = s.replace(/[\[\]]/g, "");               // одиночные квадратные скобки
  const drop = /(https?:\/\/|t\.me\/|www\.|@[a-z0-9_]{3,}|в\s*личк|в\s*описани|профил|канал|подбор|по\s*всем\s*вопрос|[📞☎])/iu;
  return s
    .split(/\r?\n/)
    .map((l) => l.replace(/#[\p{L}][^\s#]*/gu, "").trim()) // буквенные хэштеги (#11 — номер дома — сохраняем)
    .filter((l) => l && /[\p{L}\p{N}]/u.test(l) && !drop.test(l))
    .join("\n")
    .replace(/[ \t]{2,}/g, " ")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
