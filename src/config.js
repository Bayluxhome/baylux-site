...
// Абсолютный адрес сайта — для ссылок в сообщениях (Telegram, WhatsApp), подборок, OG.
// Одно место вместо «https://bayluxhome.com» строкой по коду: при смене домена меняется здесь.
export const SITE_URL = (process.env.NEXT_PUBLIC_SITE_URL || "https://bayluxhome.com").replace(/\/+$/, "");
...
// Оператор персональных данных (юридическая идентификация). Пока физлицо — до регистрации ШПС.
// В публичном футере не показывается (там бренд), но остаётся в Политике конфиденциальности:
// сведения об операторе обязательны, убирать их без юридической проверки нельзя.
...
// Пустое значение = сведения не подтверждены, блок не показывается (заглушек в проде быть не должно).
// ⚠️ Подтвердить у учредителей: адрес офиса, корпоративная почта (сейчас — единственный подтверждённый gmail),
//    юридическое название после регистрации ШПС.
export const COMPANY = {
  publicName: "Baylux Home",
  legalName: "",              // после регистрации ШПС
  officeCity: "Батуми",       // ключ CITY_TR — локализуется через cityLabel
  officeAddress: "",          // точный адрес офиса — не подтверждён, не показываем
  officeMapUrl: "",           // ссылка на маршрут — только при подтверждённом адресе
  phone: PHONE,               // рабочий телефон (без +), tel: строится из него
  email: "bayluxhome@gmail.com", // корпоративная почта — подтвердить/заменить
  telegram: TG_CONTACT,       // username без @
  businessHours: "",          // не подтверждены — не показываем
};
